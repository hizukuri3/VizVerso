...
import type { TableauDocument, TableauDatasource, TableauWorksheet, TableauDashboard, TableauField, WorksheetPane } from '../types/tableau';
...
export function parseTableauXml(xmlString: string): TableauDocument {
  const parser = new XMLParser({
    ...
    isArray: (name) => ['datasource', 'worksheet', 'dashboard', 'column', 'column-instance', 'zone'].includes(name),
  });
  ...
  try {
    ...
    if (xmlString) {
      console.log('Problematic XML Snippet (First 1000 chars):', xmlString.substring(0, 1000));
    }
  ...
  wsList.forEach((ws: any) => {
    ...
    if (view) {
      // 依存データソース名を収集
      ensureArray(view?.datasources?.datasource).forEach((ds: any) => {
        const dsName = strip(ds['@_name']);
      ...
      ensureArray(view['datasource-dependencies']).forEach((dep: any) => {
        // 1. column (ローカル計算フィールドなど)
        ensureArray(dep.column).forEach((col: any) => {
          const colName = strip(col['@_name']);
          if (colName) {
        ...
        ensureArray(dep['column-instance']).forEach((ci: any) => {
          const ciName = strip(ci['@_name']);
          const sourceColName = strip(ci['@_column']);
          if (ciName) {
    ...
    const parseFieldRefObj = (raw: string | undefined) => {
      ...
      if (bracketMatches.length > 0) {
        ...
        const filtered = parts.filter(p => !typeIdentifiers.includes(p.toLowerCase()));
    ...
    const rowsRaw: string = typeof table?.rows === 'string' ? table.rows : (table?.rows?.['#text'] || '');
    const colsRaw: string = typeof table?.cols === 'string' ? table.cols : (table?.cols?.['#text'] || '');
    ...
    ensureArray(view?.filter).forEach((f: any) => {
      const obj = parseFieldRefObj(f['@_column']);
      if (obj && !filterFields.some(x => x.name === obj.name)) filterFields.push(obj);
    });
    ...
    panes.forEach((p: any) => {
      const id = p['@_id'];
      const yAxisName = p['@_y-axis-name'];
      const xAxisName = p['@_x-axis-name'];
      ...
      const findName = (obj: any): string | undefined => {
        ...
        // mp. で始まる属性があれば最優先
        for (const k in obj) {
          if (typeof obj[k] === 'string' && obj[k].includes('mp.')) return obj[k];
        }
      ...
      const isInternalName = (name: string | undefined) => {
        if (!name) return true;
        const internalNames = ['selection-relaxation-allow', 'selection-relaxation-option', 'none', 'true', 'false'];
        return internalNames.includes(name.toLowerCase()) || name.includes('__INTERNAL__');
      };
      ...
      if (!rawMark || rawMark.toLowerCase() === 'automatic') {
        ...
        const hasDate = allShelfNames.some(n => {
          const parts = n.split(':');
          const last = parts[parts.length - 1].replace(/^\[/, '').replace(/\]$/, '');
          const localF = localFields.find(f => f.column === last);
          return localF?.dataType === 'date' || localF?.dataType === 'datetime';
        });
        const hasGeo = allShelfNames.some(n => n.toLowerCase().includes('latitude') || n.toLowerCase().includes('longitude'));
        const continuousCount = [...rowFields, ...colFields].filter(f => f.isContinuous).length;
      ...
      if (enc) {
        const getCols = (v: any) => ensureArray(v)
          .map((e: any) => {
            const raw = e['@_column'];
            if (!raw) return null;
            return {
              name: raw,
              isContinuous: raw.includes(':qk') || raw.includes(':ok') || raw.includes(':ok2') || raw.includes(':ni') || raw.includes(':oi')
            };
          })
          .filter((f): f is { name: string; isContinuous: boolean } => f !== null);
  ...
  dbList.forEach((db: any) => {
    const name = strip(db['@_name']);
    const caption = db['@_caption'] || undefined;
    const wSheets: string[] = [];

    const findSheetsInZones = (obj: any) => {
      if (!obj || typeof obj !== 'object') return;
      
      // zone 要素があれば処理
      const zones = ensureArray(obj.zone || obj);
      zones.forEach((zone: any) => {
        if (!zone || typeof zone !== 'object') return;
        
        const zoneName = strip(zone['@_name']);
        ...
        for (const key in zone) {
          if (typeof zone[key] === 'object') {
            findSheetsInZones(zone[key]);
          }
        }
      });
    };
 ...
 * 例: "[Sales]" → "Sales",  "[Superstore].[Sales]" → "Sales"
 */
function strip(name: string | undefined): string {
  ...
  return (filtered[filtered.length - 1] || pts[pts.length - 1] || n).replace(/^\[/, '').replace(/\]$/, '').trim();
}

function ensureArray(obj: any): any[] {
  if (!obj) return [];
