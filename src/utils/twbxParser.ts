 ...
 */
export async function extractTwbFromTwbx(file: File | Blob, options?: TwbxParseOptions): Promise<string> {
  ...
  // .twb ファイルを探す
  const twbFileName = Object.keys(zip.files).find(name => name.endsWith('.twb'));
