import { Layout, FileText, Database, Filter, MousePointer2, Hash } from 'lucide-react';
...
interface DetailViewProps {
  ...
  onNavigate?: (type: 'dashboard' | 'worksheet' | 'datasource', id: string) => void;
}

export default function DetailView({ doc, selectedId, selectedType, onNavigate }: DetailViewProps) {
  // メタデータ構築 (全ビューで利用可能にする)
  const fieldMeta = new Map<string, { caption?: string; isCalc: boolean; formula?: string; isContinuous?: boolean; type?: string; dataType?: string }>();
  ...
  if (selectedType === 'worksheet') {
    ...
    ws?.localFields?.forEach(f => {
      ...
      fieldMeta.set(f.column, { 
        ...
        isContinuous: f.type === 'quantitative' || f.isContinuous || (fieldMeta.get(f.column)?.isContinuous ?? false), 
  ...
  const getCaption = (fieldName: string) => {
    ...
    if (/^(nk|qk|ok|ok2|ni|oi)$/i.test(last)) {
  ...
  const formatFormulaText = (rawFormula: string | undefined) => {
    ...
    return decoded.replace(/\[(?:([^\]]+)\]\.\[)?([^\]]+)\]/g, (_match, dsName, fieldName) => {
      const caption = getCaption(fieldName);
      if (dsName === 'Parameters' || dsName === 'パラメーター') return `[パラメーター].[${caption}]`;
      return `[${caption}]`;
    });
  };
    ...
    return (
      <div className="flex-1 flex flex-col items-center justify-center text-slate-400 bg-slate-50/30">
        <div className="p-6 bg-white rounded-2xl shadow-sm border border-slate-100 flex flex-col items-center gap-4">
          ...
          <p className="text-sm font-medium">ナビゲーターからアイテムを選択してください</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="flex-1 overflow-y-auto p-10 space-y-10 animate-in fade-in slide-in-from-right-4 duration-500">
        <header className="flex items-center gap-6">
          ...
          <div>
            <h1 className="text-4xl font-black text-slate-800 tracking-tight">{db.name}</h1>
            <p className="text-slate-500 font-medium text-lg mt-1">ダッシュボードの概要</p>
          </div>
        </header>

        <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-100">
            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4">構成内容</h3>
            <p className="text-5xl font-black text-slate-800">{db.worksheets.length} <span className="text-xl font-medium text-slate-400 ml-1">シート</span></p>
          </div>
        </section>

        <section>
          <h3 className="text-sm font-bold text-slate-800 mb-6 flex items-center gap-3">
            <FileText size={20} className="text-emerald-500" /> このダッシュボード内のワークシート
          </h3>
            ...
            {db.worksheets.map(wsName => {
              ...
              return (
                <div 
                  ...
                  className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm hover:border-blue-400 hover:shadow-md transition-all cursor-pointer group active:scale-[0.98]"
                >
                  <p className="font-bold text-slate-700 group-hover:text-blue-600 transition-colors">{displayName}</p>
                  <p className="text-[10px] text-slate-400 mt-1 uppercase tracking-wider font-bold">詳細を表示 →</p>
                </div>
              );
            })}
          </div>
  ...
  if (selectedType === 'worksheet') {
    ...
    const getFieldInfo = (fieldName: string, shelfContinuous?: boolean) => {
      ...
      const isSum = /\bsum:/i.test(fieldName) && !displayCaption.includes('合計');
      const isAvg = /\bavg:/i.test(fieldName) && !displayCaption.includes('平均');
      const isMin = /\bmin:/i.test(fieldName) && !displayCaption.includes('最小');
      const isMax = /\bmax:/i.test(fieldName) && !displayCaption.includes('最大');
      const isCount = (/\bcnt:|\bcntd:/i.test(fieldName)) && !displayCaption.includes('カウント');
      const isAttr = /\battr:/i.test(fieldName) && !displayCaption.includes('属性');
      const isCollect = (/\bcollect:|\bspatial:/i.test(fieldName) || meta?.dataType === 'spatial') && !displayCaption.includes('収集');
      const isTableCalc = (fieldName.includes('rank:') || fieldName.includes('running:') || fieldName.includes('window:')) && !displayCaption.includes('△');

      let formattedCaption = displayCaption;
      if (!formattedCaption.startsWith('[') && !/^\d+$/.test(formattedCaption) && !formattedCaption.includes('(')) {
      ...
      return {
        ...
        isContinuous: shelfContinuous !== undefined ? shelfContinuous : (meta?.isContinuous ?? false),
      };
    };

    const renderShelfCard = (title: string, fields: {name: string, isContinuous?: boolean}[] | undefined, icon: React.ReactNode, colorClass: string) => {
      const hasFields = fields && fields.length > 0;
      return (
        <div className={`bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col h-full`}>
          <div className={`px-4 py-3 border-b border-slate-100 flex items-center gap-2 font-bold text-xs uppercase tracking-wider ${colorClass}`}>
            {icon} {title}
          </div>
          <div className="p-5 flex flex-wrap flex-1 content-start gap-2">
            {hasFields ? (
              ...
              <span className="text-[11px] text-slate-300 italic py-1">（なし）</span>
            )}
          </div>
        </div>
      );
    ...
    const renderPane = (pane: WorksheetPane, index: number) => {
      // 全フィールドが空の場合は非表示
      const allEmpty = Object.values(pane.encodings).every(arr => arr.length === 0);
      if (allEmpty) return null;

      // マップレイヤー判定
      const isMapChart = ws.shelf?.panes && ws.shelf.panes.some(p => p.name?.includes('mp.'));
      ...
      const hasAllPane = ws.shelf?.panes && ws.shelf.panes.length > splitMeasures.length;
        ...
        if (hasAllPane && index === 0) {
          ...
          if (pane.yAxisName || pane.xAxisName) {
            ...
            headerLabel = matchedField
              ? getFieldInfo(matchedField.name, matchedField.isContinuous).caption
              : `${t('marks')} ${index + 1}`;
          }
        }
      } else {
      ...
      return (
        <div key={index} className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col h-full">
          <div className="px-4 py-3 border-b border-slate-100 flex items-center justify-between bg-pink-50/30">
            ...
            {pane.markType !== undefined && (
              <span className="text-[10px] font-semibold text-slate-400 bg-slate-100 px-2 py-0.5 rounded-full">{tMark(pane.markType)}</span>
            )}
          </div>
          <div className="p-5 space-y-6 flex-1">
            {[
              ...
              <div key={group.label}>
                <p className="text-[10px] font-bold text-slate-400 uppercase mb-3 tracking-widest">{group.label}</p>
                <div className="flex flex-wrap min-h-[1.5rem] gap-1">
                  {group.fields && group.fields.length > 0 ? (
                    group.fields.map((f, i) => <Pill key={`${f.name}-${i}`} {...getFieldInfo(f.name, f.isContinuous)} />)
                  ) : (
                    <span className="text-[10px] text-slate-200 italic">（なし）</span>
                  )}
                </div>
              </div>
            ))}
    ...
    return (
      <div className="flex-1 overflow-y-auto p-10 space-y-10 animate-in fade-in slide-in-from-right-4 duration-500">
        <header className="flex items-center gap-6">
          ...
          <div>
            <h1 className="text-4xl font-black text-slate-800 tracking-tight">{ws.caption || ws.name}</h1>
            <div className="flex items-center gap-3 mt-2">
              <span className="px-2 py-1 bg-slate-100 text-slate-500 text-[10px] font-bold rounded uppercase tracking-widest">{t('worksheet')}</span>
              <span className="text-slate-200">/</span>
              <span className="text-slate-500 text-sm flex items-center gap-1.5 font-medium">
                <Database size={14} className="text-slate-400"/> {ws.datasourceNames?.join(', ')}
              </span>
            </div>
          </div>
        </header>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-8 items-stretch">
          ...
          <div className="grid grid-cols-1 gap-6 content-start">
            {renderShelfCard(t('columns'), ws.shelf?.cols, <Layout size={14} className="rotate-90" />, "bg-blue-50/50 text-blue-700")}
            {renderShelfCard(t('rows'), ws.shelf?.rows, <Layout size={14} />, "bg-indigo-50/50 text-indigo-700")}
            {renderShelfCard(t('filters'), ws.shelf?.filters, <Filter size={14} />, "bg-amber-50/50 text-amber-700")}
          </div>
    ...
    return (
      <div className="flex-1 overflow-y-auto p-10 space-y-10 animate-in fade-in slide-in-from-right-4 duration-500">
        <header className="flex items-center gap-6">
          ...
          <div>
            <h1 className="text-4xl font-black text-slate-800 tracking-tight">{ds.caption || ds.name}</h1>
            <p className="text-slate-500 font-medium text-lg mt-1">Data Source Fields</p>
          </div>
        </header>
        ...
        <section className="space-y-8">
          ...
          <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
            ...
            <div className="p-8 flex flex-wrap gap-2">
              {normal.map(f => (
                <Pill key={f.column} name={f.column} caption={f.caption} isContinuous={f.type === 'quantitative'} />
              ))}
            </div>
          </div>
        </section>
