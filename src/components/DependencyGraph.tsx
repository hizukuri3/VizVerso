...
export default function DependencyGraph({ nodes: initialNodes, edges: initialEdges }: DependencyGraphProps) {
  ...
  useEffect(() => {
    ...
    setNodes(initialNodes.map((n) => ({ ...n, hidden: !visibleNodeIds.has(n.id) })));
    setEdges(initialEdges.map((e) => ({ ...e, hidden: !visibleEdgeIds.has(e.id) })));
