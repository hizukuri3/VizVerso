...
export default function Sidebar({ doc, selectedId, onSelect }: SidebarProps) {
  ...
  const handleItemClick = (e: React.MouseEvent, type: 'dashboard' | 'worksheet' | 'datasource', id: string) => {
    e.stopPropagation();
    onSelect(type, id);
  };

  // すべてのダッシュボードに含まれるシート名を収集
  const allDashboardSheets = new Set(doc.dashboards.flatMap(db => db.worksheets));

  // ダッシュボードに含まれない「浮いた」シートを抽出
  const floatingSheets = doc.worksheets.filter(ws => !allDashboardSheets.has(ws.name));
                  ...
                  <div key={db.name}>
                    <button
                      ...
                      className={`w-full text-left px-3 py-2 rounded-xl text-sm transition-all duration-200 flex items-center gap-2 group ${
                        isActive ? 'sidebar-item-active shadow-sm' : 'hover:bg-slate-50 text-slate-600'
                      }`}
                    >
                      ...
                      <div className="ml-6 mt-1 space-y-1 border-l-2 border-slate-100 pl-3 animate-in slide-in-from-top-1 duration-200">
                        {db.worksheets.map(wsName => {
                          ...
                          return (
                            <button
                              key={wsName}
                              onClick={(e) => handleItemClick(e, 'worksheet', wsName)}
                              className={`w-full text-left px-3 py-1.5 rounded-lg text-xs transition-colors flex items-center gap-2 ${
                                selectedId === wsName ? 'bg-blue-50 text-blue-700 font-bold' : 'text-slate-500 hover:bg-slate-50 hover:text-blue-600'
                              }`}
                            >
                              <FileText size={12} className={selectedId === wsName ? 'text-blue-500' : 'text-slate-300'} />
            ...
            <div className="space-y-1">
              {floatingSheets.map((ws) => (
                <button
                  ...
                  className={`w-full text-left px-3 py-2 rounded-xl text-sm transition-all duration-200 flex items-center gap-2 group ${
                    selectedId === ws.name ? 'sidebar-item-active shadow-sm' : 'hover:bg-slate-50 text-slate-600'
                  }`}
                >
                  <FileText size={14} className={selectedId === ws.name ? 'text-blue-500' : 'text-slate-300'} />
                  <span className="truncate font-semibold">{ws.caption || ws.name}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="space-y-1">
            {doc.datasources.map((ds) => (
              <button
                ...
                className={`w-full text-left px-3 py-2 rounded-xl text-sm transition-all duration-200 flex items-center gap-2 group ${
                  selectedId === ds.name ? 'sidebar-item-active shadow-sm' : 'hover:bg-slate-50 text-slate-600'
                }`}
              >
                <Database size={14} className={selectedId === ds.name ? 'text-blue-500' : 'text-slate-300'} />
                <span className="truncate font-semibold">{ds.caption || ds.name}</span>
              </button>
            ))}
          </div>
        </div>
